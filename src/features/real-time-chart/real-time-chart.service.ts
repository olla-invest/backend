...
export class RealTimeChartService implements OnModuleInit {
  ...
  async getStockSummary(stockCode: string) {
    ...
    // latestDbCandle이 오늘 날짜인지 확인 — 아니면 DB 종가를 현재가/전일대비 계산에 쓰지 않음
    const todayStr = `${today.slice(0, 4)}-${today.slice(4, 6)}-${today.slice(6, 8)}`;
    const latestDbIsToday = latestDbCandle
      ? new Date(latestDbCandle.candleTime).toISOString().slice(0, 10) === todayStr
      : false;

    const realtimeCurrentPrice = Number(realtimePrice?.currentPrice ?? 0);
    const dbCurrentPrice = latestDbIsToday && latestDbCandle ? Number(latestDbCandle.closePrice) : 0;
    ...
    // latestDbCandle이 오늘이어야 previousDbCandle이 어제(전일) 종가가 됨
    const previousDbClose = latestDbIsToday && previousDbCandle ? Number(previousDbCandle.closePrice) : null;
    ...
    const prevDayCompare = Math.round(
  ...
  async getStockListWithRangeRS(
    ...
    // KOSPI/KOSDAQ 분리 병렬 계산 — calculateRangeRS는 필요한 날짜 포인트만 로드하므로
    // calculateRsFilterLog(전체 365일 로드 + 로그 파일 쓰기)보다 훨씬 가볍다
    const kospiStocks = validStocks.filter((s) => this.isKospiStock(s)).map((s) => s.code);
    const kosdaqStocks = validStocks.filter((s) => this.isKosdaqStock(s)).map((s) => s.code);

    const [kospiRS, kosdaqRS, latestTradeDate, periods, metricsMap] = await Promise.all([
      kospiStocks.length > 0
        ? this.metricsService.calculateRangeRS(kospiStocks, rsFilters, 'INDEX_KOSPI', 4)
        : Promise.resolve(new Map<string, Array<{ tradeDate: Date; rank: number; rsScore: number }>>()),
      kosdaqStocks.length > 0
        ? this.metricsService.calculateRangeRS(kosdaqStocks, rsFilters, 'INDEX_KOSDAQ', 4)
        : Promise.resolve(new Map<string, Array<{ tradeDate: Date; rank: number; rsScore: number }>>()),
      this.metricsService.getLatestTradeDate(),
      this.metricsService.resolveTradingPeriodsFromRanges(rsFilters, 'INDEX_KOSPI'),
      this.metricsService.getLatestMetrics(allStockCodes),
    ]);

    const weights = rsFilters.map((filter) => filter.strength);
    const rsHistoryMap: Map<string, Array<{ tradeDate: Date; rank: number; rsScore: number }>> = new Map([...kospiRS, ...kosdaqRS]);

    this.logger.log(`Range RS calculated: KOSPI=${kospiRS.size}, KOSDAQ=${kosdaqRS.size} stocks. periods=${periods}, weights=${weights}`);
  ...
  /**
   * 단일 종목의 일봉 비수정가/수정주가를 모두 수집해 stockCandle 두 필드에 저장
   *
   *
